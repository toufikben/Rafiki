...
| 3D renderer (flutter_scene) | Prototype scope; leak | ✅ `DogSceneView.dispose` never disposes the Scene / removes nodes; `initializeStaticResources()` re-runs per widget instance; animation runtime is a designed no-op until authored clips exist |
