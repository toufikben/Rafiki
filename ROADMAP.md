...
**Status: implementation gate closed; production-asset gate pending.** A self-contained original stylized dog GLB is committed and loads through `DogSceneView`. Measured 2026-09-25 from the binary: valid glTF 2.0, 113,456 bytes, 21 named nodes, 20 primitives (~4,920 triangles, inside the 12k target), 5 materials, 1 embedded texture, **0 animations and 0 skins**. Known contract gaps: no jaw, no eyelids, only `Tail`/`TailTip` instead of the `Tail_01..03` chain, and `Paw` reused 4 times instead of unique paw joints. A testable `DogAnimationController`, manifest contract validator, and runtime `DogAnimationRuntime` clip binder with crossfade support are integrated. The runtime is safe when the current prototype has no authored clips and will bind `Idle`, `Walk`, `Run`, `Play`, `Sleep`, `Eat`, `Drink`, `Happy`, and `Sad` when the production GLB supplies them.
...
**Status: in progress; core management and quality slice implemented. Priority: high.** Added an explicit settings flow backed by `LocalModelManager` for listing installed models, selecting a local `.litertlm` file with `file_picker`, installing from a user-supplied URL, progress reporting, cancellation, retry, uninstall and orphan cleanup. The UI also shows installed storage usage, active model identity and a Wi-Fi/mobile-data warning. A preflight layer now validates extension, existence, minimum local size and remote HTTP(S) URL/size metadata before installation. The settings catalog (`lib/ai/recommended_models.dart`) now names real engine-supported options with **Qwen3 0.6B as the best-quality default**, ahead of balanced LFM2.5 230M and lightest SmolLM2 135M; no binary is bundled and no download happens implicitly. The first-run path remains offline and never downloads implicitly. The deterministic fallback and privacy boundary are covered by tests. Chat now detects Arabic/English scripts, bounds native history by recreating the session after 12 turns and exposes a stop action that closes the active generation session.

**Remaining work:** improve platform-specific picker permissions and error copy, add a richer free-space confirmation when the platform exposes it, and perform model-runtime failure injection plus chat-generation verification on Android with a real installed Qwen3 0.6B build over adb (no Flutter/Android toolchain exists in the current automation environment, so this stays a field gate with CI analysis/tests as the interim check). The URL download flow now performs preflight and requires an explicit confirmation dialog showing model name, estimated size, current model storage, active model, data-use warning and cancel/download actions.
...
**Remaining work:** provide real production identifiers through the release environment (including replacing the sample AdMob application ID in `AndroidManifest.xml` with the production app ID at release time — never committed), select and configure a consent/crash-reporting provider, complete localization and accessibility review, and create a release-signed build outside the repository. The debug build path is ready for APK inspection; the CI release job is allowed to fail until signing and secrets are configured. The build commands and secret boundary are documented in [`docs/RELEASE_BUILD.md`](docs/RELEASE_BUILD.md).
